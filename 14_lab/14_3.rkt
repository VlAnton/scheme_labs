...
(define (count_maxes tree)
  (define (rec tree current_max counter)
    (cond
      ...
      [(leaf? tree)
       (cond
         [(= current_max (car tree)) (cons current_max (+ counter 1))]
         [(< current_max (car tree)) (cons (car tree) 1)]
      ...
      [else
       (let* ([node (car tree)]
              [curr_max_data
               (cond
                 [(= node current_max) (cons current_max (+ counter 1))]
                 [(> node current_max) (cons node 1)]
                 [else (cons current_max counter)]
                 )
               ]
